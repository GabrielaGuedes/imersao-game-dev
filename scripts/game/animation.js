class Animation {
  constructor(imageConfigs, x, y) {
    ...
    this.x = x;
    this.y = y || height - this.height - GAME_CONSTANTS.floorHeight;
  ...
  polygonFrame() {
    ...
    polygonFrame[2] = createVector(
      adjustedX,
      adjustedY + (2 * adjustedHeight) / 3
    );
    polygonFrame[3] = createVector(
      adjustedX + adjustedWidth / 3,
      adjustedY + adjustedHeight
    );
    polygonFrame[4] = createVector(
      adjustedX + (2 * adjustedWidth) / 3,
      adjustedY + adjustedHeight
    );
    polygonFrame[5] = createVector(
      adjustedX + adjustedWidth,
      adjustedY + (2 * adjustedHeight) / 3
    );
    polygonFrame[6] = createVector(
      adjustedX + adjustedWidth,
      adjustedY + adjustedHeight / 3
    );
    polygonFrame[7] = createVector(
      adjustedX + (2 * adjustedWidth) / 3,
      adjustedY
    );
  ...
  _calculateMatrix() {
    ...
    for (let i = 0; i < this.imageFrames; i++) {
      this.matrix[i] = [
        (i % this.frameColumns) * this.spriteWidth,
        parseInt(i / this.frameColumns) * this.spriteHeight,
      ];
    }
  }
}
